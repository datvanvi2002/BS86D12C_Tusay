...
{
    while(!_txif0); //waitting transmit free
	volatile unsigned char s = _u0sr; //theo sequence
    (void)s;
	_txr_rxr0 = data; //write data to uart transmit
	while(!_tidle0); //waitting transmit OK
}


void uart0_send_string(const char *s)
{
    while (*s) uart0_send_byte((unsigned char)*s++);
}

void uart0_send_number(uint16_t x){
    char buf[6]; int i = 0;
    if (x == 0){ uart0_send_byte('0'); return; }
	while (x > 0){
		buf[i++] = (char)((x % 10) + '0');
		x /= 10;
	}
    while (i--) uart0_send_byte(buf[i]);
}