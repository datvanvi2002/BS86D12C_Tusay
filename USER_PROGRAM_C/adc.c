...
void adc_init_vdd_ref(void) {
    ...
    _sadc1  = 0x0B;     // SAINS=000 | SAVRS=01 (VDD) | SACKS=011 (fSYS/8)

    _sadc0  = 0x10;     // ADRFS=1 (right-aligned)
...
unsigned int adc_read_channel(unsigned char channel)
{
    ...
    while (_sadc0 & 0x40) { ; }         // đợi ADBZ=0 (xong chuyển đổi)

    return ((unsigned int)_sadoh << 8) | _sadol;  // ADRFS=1: right-aligned
}
static inline uint16_t adc_to_mV(uint16_t adc12, uint16_t vdd_mV) {
    return (uint32_t)adc12 * vdd_mV / 4095u;
}

int pressure_sensor_read_mbar(uint16_t adc_value) {
    // Giả sử cảm biến áp suất có đầu ra tuyến tính từ 0.5V (0 mbar) đến 4.5V (1000 mbar)
    // Với VDD = 5V, ta có thể tính áp suất như sau:
    float voltage = ((float)adc_value) * 5.0f / 4095.0f; // Chuyển đổi ADC sang điện áp
    if (voltage < 0.5f) voltage = 0.5f;                  // Giới hạn dưới
    if (voltage > 4.5f) voltage = 4.5f;                  // Giới hạn trên
    float pressure_mbar = (voltage - 0.5f) * (1000.0f / (4.5f - 0.5f)); // Tính áp suất
